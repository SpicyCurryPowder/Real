...
const pageNewsRouter = express.Router();
const axios = require('axios');

pageNewsRouter.get('', async(req, res) => {
    try {
        ...
        const article = articles.find(a => a.article_id === articleId) || articles[2] || articles[1] || articles[2] || articles[3] || articles[4] || articles[5] || articles[6] || articles[7] || articles[8] || articles[9];
        // console.log('Selected article:', article);

        res.render('newspage-verified', { article: article });
    } catch (error) {
        console.error('Error fetching verified news:', error);
        res.render('newspage-verified', { article: {} });
    }
});



module.exports = pageNewsRouter;